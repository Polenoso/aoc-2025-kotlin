...
fun main() {
    data class Point3D(val x: Int, val y: Int, val z: Int) {
        ...
        constructor(coordString: String) : this(
            coordString.split(",")[0].toInt(),
            coordString.split(",")[1].toInt(),
            coordString.split(",")[2].toInt()
        )
    ...
    fun part1(input: List<String>, iterations: Int = 10): Long {
        val points = input.map(::Point3D)
        val n = points.size
        // Generar todos los pares posibles con su distancia
        data class Edge(val i: Int, val j: Int, val dist: Double)
        val edges = mutableListOf<Edge>()
        for (i in 0 until n) {
            for (j in i + 1 until n) {
                edges.add(Edge(i, j, points[i].distance(points[j])))
            }
        }
        // Ordenar los pares por distancia
        edges.sortBy { it.dist }
        val circuits = mutableListOf<MutableSet<Point3D>>()

        for (iter in 0 until iterations) {
            val edge = edges[iter]
            val p1 = points[edge.i]
            val p2 = points[edge.j]
    ...
    fun part2(input: List<String>): Long {
        ...
        edges.sortBy { it.dist }
        val circuits = mutableListOf<MutableSet<Point3D>>()
        circuits.addAll(points.map { mutableSetOf(it) })
        var lastEdge: Edge = edges[0]
        ...
        return points[lastEdge.i].x.toLong() * points[lastEdge.j].x.toLong()
    }
